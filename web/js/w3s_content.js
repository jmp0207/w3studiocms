...
var w3sContent = Class.create({
	...
	moveContents1: function(idSlot, slotName, params)
	{   
	  /* Params if empty when the user moves the last content for a slot 
	   * or when a user does a sort.
	   */
	  if (params == '') params = 'none';
					  
	  /* Builds an array with the moving parametes and the moveCounter variable
	   * is used as array name. In fact here two arrays are builded the one 
	   * mentioned above and the one passed as reference with the params
	   * variable. You can notice that this variable doens't belong the 
	   * moveCounter array.
	   */
	  W3sContent.moveParameters += W3sContent.moveCounter + '[]=' + idSlot + '&' + W3sContent.moveCounter + '[]=' + slotName + '&' + W3sContent.moveCounter + '[]=' + InteractiveMenu.idContent + '&' + params + '&';
	  
	  // Completes the movement
	  if (W3sContent.moveCounter == 2){
			
	    // Stops the PeriodicalUpdater (more info below)
	    pe.stop();
	    ...
	    {asynchronous:true,
	     ...
	     parameters:W3sContent.moveParameters +
	                'page=' + w3studioCMS.page +
	                '&language=' + w3studioCMS.language});
	    
	    // Resets the support variables  
	    W3sContent.moveParameters = '';
	    W3sContent.moveCounter = 0; 
	  }
	  else if (W3sContent.moveCounter == 1){ 
	    
	    /* If user does only a sort of contents the routine is called only a time
	     * and the movement cannot be completed. In this case a PeriodicalExecuter
	     * is started and if one second passes it calls the movePageItem againg
	     * to complete the moving operation. 
	     *
	     * NOTE: This is my solution to the problem but has a little problem: 
	     * when you sort the operation is immediatly made on page but the db
	     * saving is delayed of one second. If you have a best one, implements it 
	     * and that will be appreciated!
	     */
	    pe = new PeriodicalExecuter(function onlyASortMoving()
			       { 
			         pe.stop();
			         if (W3sContent.moveCounter == 2) W3sContent.moveContents(0, '', '');
			       }
			       , 1);
	  }
	  
	  W3sContent.moveCounter++;
	}
